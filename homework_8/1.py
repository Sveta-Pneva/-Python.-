...
def haffman_code_key(tree, root=0, path=''):
    # Если элемент не словарь, значит мы достигли самого символа
    # и заносим его, а так же его код в словарь (кодовую таблицу).
    if root == 0:
        root = tree[1]
    ...
    # по первому и второму значению (левая и правая ветви).
        ...
        haffman_code_key(tree[0], root, path=f'{path}0')
        haffman_code_key(tree[1], root, path=f'{path}1')
    code = code_table
    return code, root
...
print()"""
s = "beep boop beer fuuh!"
l, r = haffman_code_key(haffman_tree(s))
p = s
haffman_code(haffman_tree(p))
print(l)
for i in p:
    print(code_table[i], end=' ')
